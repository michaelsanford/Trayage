...
5. *(Optional, recommended)* Under **Application logo**, upload
   `assets/oauth/trayage-oauth-512.png` so the authorization screen shows the Trayage
   mark. (Generated by `tools/Build-TrayageIcons.ps1`; regenerate if the glyph changes.)
...
4. Copy the **Client ID** (older Bitbucket UIs label this the **Key**) into `Bitbucket:Key`,
   and the **Secret** into `Bitbucket:Secret`. (Bitbucket OAuth consumers have no logo field.)
