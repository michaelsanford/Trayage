...
<#
.SYNOPSIS
    Verify a Trayage release: GitHub build-provenance attestation, cosign keyless
    signature, and CycloneDX SBOM integrity.

.PARAMETER Repo
    GitHub owner/repo slug. Default: michaelsanford/Trayage
...
.EXAMPLE
    .\Verify-Release.ps1
    .\Verify-Release.ps1 -Tag v1.0.0
#>
param(
    [string]$Repo  = "michaelsanford/Trayage",
    [string]$Tag   = "",
    [switch]$Force
)
...
# Trayage ships one self-contained .zip per architecture (each with a sibling
# cosign .bundle) plus a single CycloneDX SBOM.
$archives = Get-ChildItem $downloadDir -Filter "*.zip" | Sort-Object Name
$sbomFile = Get-ChildItem $downloadDir -Filter "*-sbom.cdx.json" | Select-Object -First 1

if ($archives.Count -eq 0) {
    Write-Fail "No .zip archives found in downloads/"
    exit 1
}
...
foreach ($zip in $archives) {
    Write-Info "Subject: $($zip.Name)"
    $out = & gh attestation verify $zip.FullName --repo $Repo 2>&1
...
# The certificate identity is the exact workflow ref that signed the archive.
...
foreach ($zip in $archives) {
    $bundlePath = Join-Path $downloadDir "$($zip.Name).bundle"

    if (-not (Test-Path $bundlePath)) {
        Write-Fail "$($zip.Name) — bundle file not found: $($zip.Name).bundle"
        $cosignPassed = $false
        continue
    }
...
if (-not $sbomFile) {
    ...
    if ($null -ne $sbom) {
        ...
        # d) metadata.component.version present. For .NET, CycloneDX records the
        #    assembly/package version (e.g. 1.0.0), not the git tag, so a mismatch
        #    is informational rather than a failure.
        $sbomVersion = $sbom.metadata?.component?.version
        if ($sbomVersion) {
            Write-Pass "metadata.component.version = $sbomVersion"
            if ($sbomVersion -ne ($Tag -replace '^v', '')) {
                Write-Info "(reflects the assembly version, not the git tag — expected for .NET SBOMs)"
            }
        } else {
        ...
        # f) cyclonedx-cli schema validation (when installed)
        if ($hasCycloneDX) {
